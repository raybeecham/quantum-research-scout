...
const formatShortDate = value => {
  ...
  }).format(new Date(dateOnly ? `${text}T00:00:00Z` : text));
};
...
const friendlyReportName = (name, type) => {
  ...
  if (type === "MONTHLY" && monthly) {
    return new Intl.DateTimeFormat(undefined, {
      ...
      year: "numeric",
      timeZone: "UTC",
    }).format(new Date(`${monthly[1]}-${monthly[2]}-01T00:00:00Z`));
  }
  return value.replaceAll("_", " ").replace(/-(digest|weekly|monthly)$/i, "");
};
const profileUrl = (name, kind = "entities") =>
  `entity.html?name=${encodeURIComponent(name)}&kind=${encodeURIComponent(kind)}`;
...
function persistAnalystDecisions() {
  try {
    window.localStorage.setItem(
      ANALYST_DECISION_STORAGE_KEY,
      JSON.stringify(state.analystDecisions),
    );
  } catch {
...
fetch("data/dashboard.json?v=__ASSET_VERSION__")
  .then(response => {
...
function render() {
  ...
  const missionPayload = state.data.federal_missions || {
    missions: [],
    discovery_candidates: [],
    summary: {},
  };
  const fundingPayload = state.data.federal_funding || {
    records: [],
    mission_portfolios: [],
    summary: {},
  };
  const procurementPayload = state.data.procurement_intelligence || {
    opportunities: [],
    summary: {},
  };
  ...
  const temporalPayload = state.data.temporal_intelligence || {
    summary: {},
    priority_events: [],
    upcoming: [],
  };
  const forecastPayload = state.data.strategic_forecasts || {
    summary: {},
    active_forecasts: [],
    resolved_forecasts: [],
  };
  const analystDecisionPayload = state.data.decision_center || { items: [], summary: {} };
  const dataTrustPayload = state.data.data_trust || {
    summary: {},
    collector_metrics: [],
    quarantined_evidence: [],
  };
  document.getElementById("repo-link").href = safeUrl(state.data.repository_url);
  document.getElementById("alerts-report-link").href = safeUrl(
    `${state.data.repository_url}/blob/main/reports/alerts.md`,
  );
  ...
  sourceSummary.classList.remove(
    "coverage-healthy",
    "coverage-watch",
    "coverage-degraded",
    "coverage-unknown",
  ...
  document.getElementById("footer-updated").textContent =
    `Dashboard built ${formatDate(state.data.generated_at)}`;
  renderReports(state.data.reports);
  renderAlerts(alerts, temporalPayload);
  renderDecisionCenter(analystDecisionPayload, changePayload);
  renderForecasts(forecastPayload);
  renderMissions(missionPayload, fundingPayload);
  renderFunding(fundingPayload);
  renderProcurementDocuments(procurementPayload);
  renderDecisionBriefs(decisionPayload);
  renderPursuits(pursuitPayload);
  renderEvidenceLedger(claimPayload, changePayload, temporalPayload);
  renderSignals();
  renderPatents(patentPayload);
  renderWatch();
  renderTrend();
  renderReadiness();
  renderStandards();
...
function renderAlerts(payload, temporalPayload = {}) {
  ...
  document.getElementById("alert-summary").textContent = highlights.length
    ...
    : "No material changes";
  document.getElementById("alerts-report-link").textContent =
    `View ${payload.active_count || 0} monitored conditions →`;
  document.getElementById("alert-list").innerHTML = highlights.length
    ? highlights
        .map(item => {
      ...
      <footer>${evidence}${destination}</footer>
        ...
        .join("")
    : `<div class="empty-state intelligence-quiet-state"><strong>No material changes today</strong><span>The scout is monitoring for authoritative changes, urgent opportunities, amendments, and conflicts.</span></div>`;
}

function buildBriefingHighlights(
  alertPayload,
  changePayload,
  decisionPayload,
  missionPayload,
  ...
  return (payload.priority_events || [])
    .map(item => {
      ...
      return {
        ...
        priority: ["conflict_opened", "changed_since_prior_run", "superseded"].includes(
        ...
        meta: briefingMeta(
          sourceName(item.evidence_url),
          temporal.primary_date || temporal.first_observed_at,
        ),
        evidenceUrl: item.evidence_url || "",
        destinationUrl: `${state.data.repository_url}/blob/main/reports/temporal-intelligence.md`,
        destinationLabel: "Open time trace",
        timeSensitive: ["conflict_opened", "changed_since_prior_run"].includes(
          temporal.classification,
        ),
      };
    })
...
function meaningfulChangeHighlights(payload) {
  const criticalPredicates = new Set([
    "deadline",
    "requirement",
    "qualification_gate",
    "opportunity_status",
    ...
    .flatMap(changeType =>
      (payload[changeType] || []).map(event => {
        ...
        return {
          ...
          priority,
          score:
            (priority === "critical" ? 116 : 96) + (event.authority === "authoritative" ? 8 : 0),
...
function materialAlertHighlights(payload) {
  const rules = {
    procurement_amendment_impact: {
      ...
      timeSensitive: true,
      action:
        "Revalidate requirements, risk, checklist items, and the bid decision against the amendment.",
    },
    procurement_amendment: {
      ...
      timeSensitive: true,
      action:
        "Compare the new document with the prior solicitation and identify changed requirements or dates.",
    },
    opportunity_closing: {
      label: "CLOSING NOW",
      score: 136,
      timeSensitive: true,
      action: "Review eligibility and requirements now, then record a bid/no-bid decision.",
    },
    opportunity_new: {
      label: "PRIORITY OPPORTUNITY",
      score: 92,
      timeSensitive: false,
      action: "Assess mission fit, eligibility, technical requirements, and pursuit value.",
    },
  };
    ...
    .map(alert => {
      const rule = alert.type?.startsWith("entity_")
        ? {
            ...
            timeSensitive: false,
            action:
              "Review the authoritative evidence and assess mission, technology, and competitor implications.",
          }
        : rules[alert.type];
      if (!rule) return null;
      ...
      return {
        ...
        statusLabel:
          alert.is_new && rule.label !== "CLOSING NOW" ? `NEW · ${rule.label}` : rule.label,
        priority: alert.severity || "high",
        score: rule.score + (alert.is_new ? 5 : 0),
        title: smartBriefingTitle(stripAlertPrefix(alert.title)),
        why,
        action:
          summaryParts.length > 1 && ["opportunity_closing", "opportunity_new"].includes(alert.type)
            ? summaryParts.at(-1)
            : rule.action,
        ...
        destinationLabel: alert.type?.startsWith("opportunity_")
...
function decisionHighlights(payload, temporalPayload = {}) {
  ...
  return (payload.items || [])
    ...
    .map(item => {
      ...
      const temporalEvent = (temporalPayload.priority_events || []).find(
        event => event.evidence_url && event.evidence_url === evidence.url,
      );
      ...
      const temporalBonus = ["happened_today", "published_today"].includes(temporal.classification)
        ? 16
        : ["recent_event", "recently_published"].includes(temporal.classification)
          ? 9
          : historical
            ? -32
            : 0;
      return {
        ...
        statusLabel: historical
          ...
          : labels[item.queue_type] || "ANALYST DECISION",
        priority,
        score:
          (priority === "critical" ? 118 : priority === "high" ? 98 : 78) +
          Math.min(12, Number(details.selection_score || 0) / 10) +
          decisionRecencyScore(details.record_date) +
          temporalBonus,
        title: decisionBriefingTitle(item),
        why: historical
          ? `${temporal.explanation} ${decisionBriefingWhy(item)}`
          : decisionBriefingWhy(item),
        action: decisionBriefingAction(item),
        meta: briefingMeta(
          details.awarding_agency || sourceName(evidence.url),
          details.record_date || item.observed_at,
        ),
...
function decisionRecencyScore(value) {
  ...
  const ageDays =
    (new Date(state.data.generated_at) - new Date(`${String(value).slice(0, 10)}T00:00:00Z`)) /
    86400000;
...
function decisionBriefingWhy(item) {
  ...
  if (item.queue_type === "claim_conflict")
    return (
      item.why ||
      "Authoritative evidence remains in conflict and cannot safely support a decision yet."
    );
  if (item.queue_type === "amendment_revalidation")
    return (
      item.why || "A material solicitation amendment changed a controlling pursuit assumption."
    );
...
function decisionBriefingAction(item) {
  const details = item.details || {};
  const predicates = new Set(details.predicates || [details.predicate]);
  if (item.queue_type === "claim_conflict" || item.queue_type === "amendment_revalidation")
    return item.recommended_action || "Review the evidence and record a disposition.";
  if (
    details.value === "awarded" ||
    predicates.has("opportunity_status") ||
    predicates.has("reported_recipient")
  ) {
    return "Review the recipient, value, scope, and mission linkage for competitor, partner, or procurement implications.";
  }
  return (
    item.recommended_action ||
    "Classify the signal and connect it to any affected mission or pursuit."
  );
}
...
function nextMissionMilestone(payload) {
  const milestones = (payload.missions || [])
    .map(mission => ({ mission, milestone: mission.next_milestone }))
    .filter(item => item.milestone?.target_date)
    .sort((a, b) => String(a.milestone.target_date).localeCompare(String(b.milestone.target_date)));
  ...
  return {
    ...
    meta: briefingMeta(
      (mission.lead_agencies || []).join(", ") || "Federal mission",
...
function isDuplicateHighlight(candidate, selected) {
  if (
    selected.some(
      item =>
        ...
        (item.evidenceUrl && item.evidenceUrl === candidate.evidenceUrl),
    )
  )
    return true;
  ...
  return selected.some(item => {
    ...
    return (
      overlap >= 2 && overlap / Math.max(1, Math.min(candidateTokens.length, tokens.length)) >= 0.6
    );
  });
}
...
function stripAlertPrefix(value) {
  return String(value || "").replace(
    /^(Federal opportunity closing soon|New high-priority federal opportunity|Procurement amendment impact|New procurement amendment|[^:]+ event):\s*/i,
    "",
  );
}
function titleCaseBriefing(value) {
...
function actionForPredicate(predicate) {
  return (
    {
      deadline: "Rebaseline the response calendar and confirm the controlling deadline.",
      requirement: "Update the compliance matrix and revalidate technical fit.",
      qualification_gate: "Re-run the qualification gate and confirm the pursuit decision.",
      opportunity_status: "Confirm the new status and update the pursuit workflow.",
      mission_status: "Reassess mission timing, dependencies, and linked opportunities.",
      legal_status: "Review the legal-status evidence before relying on this asset.",
    }[predicate] || "Review the before-and-after evidence and update any affected decision."
  );
}
function briefingMeta(source, date) {
  return [source || "Public evidence", briefingDate(date)].filter(Boolean).join(" · ");
}
...
function sourceName(url) {
  try {
    ...
    return (
      { "sam.gov": "SAM.gov", "grants.gov": "Grants.gov", "usaspending.gov": "USAspending" }[
        host
      ] || host
    );
  } catch {
...
function renderForecasts(payload) {
  ...
  document.getElementById("forecast-report-link").href = safeUrl(
    `${state.data.repository_url}/blob/main/reports/strategic-forecasts.md`,
  );
  ...
  document.getElementById("forecast-calibration").innerHTML = [
    ...
    [
      ...
      summary.accuracy_rate == null
        ? "Awaiting outcomes"
        : `${Math.round(Number(summary.accuracy_rate) * 100)}%`,
    ],
    [
      "Mean Brier",
      summary.mean_brier_score == null ? "Not scored" : Number(summary.mean_brier_score).toFixed(3),
    ],
  ]
    .map(
      ([label, value]) =>
    ...
    .join("");
  document.getElementById("forecast-grid").innerHTML = active.length
    ? active
        .slice(0, 4)
        .map(item => {
          ...
          const evidence = (item.evidence || [])
            ...
            .join("");
          const factors = (item.probability_factors || [])
            .map(
              factor =>
      ...
      <details><summary>Evidence, indicators, and mission dossier</summary>
        <div class="forecast-detail">
          ...
          <section><h4>Confirming indicators</h4><ul>${(item.confirming_indicators || [])
            .slice(0, 3)
            .map(value => `<li>${escapeHtml(value)}</li>`)
            .join("")}</ul></section>
          <section><h4>Disconfirming indicators</h4><ul>${(item.disconfirming_indicators || [])
            .slice(0, 3)
            .map(value => `<li>${escapeHtml(value)}</li>`)
            .join("")}</ul></section>
        ...
        .join("")
    : '<div class="empty-state">No evidence-qualified strategic forecasts are active. The registry will open hypotheses when official mission activity crosses the configured threshold.</div>';
}
...
function renderMissions(payload, fundingPayload) {
  ...
  const fundingByMission = new Map(
  ...
  document.getElementById("mission-grid").innerHTML = missions.length
    ? missions
        ...
        .map(item => {
          ...
          const fundingFact = funding?.record_count
            ? `<div><dt>Funding activity</dt><dd>${funding.record_count} record${funding.record_count === 1 ? "" : "s"} · ${funding.open_opportunities || 0} open · ${escapeHtml(formatMoney(funding.known_award_value || funding.announced_funding_value))}</dd></div>`
            : "";
      ...
      ${update ? `<a class="mission-update" href="${escapeHtml(safeUrl(update.url || item.official_url))}" target="_blank" rel="noopener">Latest: ${escapeHtml(update.title)} →</a>` : ""}
        ...
        .join("")
    : '<div class="empty-state">No federal missions are configured.</div>';

  const candidateDetails = document.getElementById("mission-candidates");
  candidateDetails.hidden = !candidates.length;
  candidateDetails.querySelector("summary").textContent =
    `${candidates.length} possible new mission${candidates.length === 1 ? "" : "s"} awaiting review`;
  document.getElementById("mission-candidate-list").innerHTML = candidates
    .map(
      item =>
...
function renderFunding(payload) {
  ...
  document.getElementById("funding-metrics").innerHTML = [
    ...
    [
      "Missions with activity",
      `${summary.missions_with_activity || 0} / ${summary.tracked_missions || 0}`,
    ],
  ...
  const visible = [...records]
    ...
    .slice(0, 6);
  document.getElementById("funding-grid").innerHTML = visible.length
    ? visible
        .map(item => {
      ...
      <footer><span>${item.related_patent_count || 0} patent match${item.related_patent_count === 1 ? "" : "es"}</span><b>${item.strategic_significance_score || 0} · ${escapeHtml(item.significance_label || "monitor")}</b></footer>
        ...
        .join("")
    : '<div class="empty-state">No funding or procurement records have been collected yet. USAspending and Grants.gov run without credentials; SAM.gov additionally requires SAM_GOV_API_KEY.</div>';
  document.getElementById("funding-portfolios").innerHTML = portfolios
    .slice(0, 6)
    .map(
      item =>
        `<span><strong>${escapeHtml(item.mission_name)}</strong>${item.record_count} records · ${item.open_opportunities} open · ${item.related_patent_count || 0} analytical patent matches</span>`,
    )
    .join("");
}
...
function renderOpportunityRadar(payload) {
  ...
  const visible = opportunities
    .filter(item => {
      if (state.opportunityFilter === "closing")
        return ["closing_soon", "closing_this_month"].includes(item.deadline_status);
    ...
    .slice(0, 8);
  document.getElementById("opportunity-radar").innerHTML = visible.length
    ? visible
        .map(item => {
          const missions = (item.mission_links || []).map(link => link.mission_name);
...
function renderDecisionCenter(payload, changePayload = {}) {
  ...
  const retainedItems = Object.values(state.analystDecisions)
    .filter(
      record =>
        ["deferred", "escalated"].includes(record?.disposition) &&
        record.item &&
        !remoteIds.has(record.decision_id),
    )
  ...
  if (!filtered.length) {
    ...
    if (state.decisionFilter === "open") {
      ...
      const clearedList = cleared.items
        .map(
          item =>
  ...
  list.innerHTML = filtered
    .map(item => {
      ...
      const evidence = labelDuplicateTitles(dedupeByUrl(item.evidence).slice(0, 4))
        .map(
          source =>
...
function recentlyClearedChanges(payload) {
  ...
  const reportedTotal =
    ...
    Number(summary.superseded || 0) +
    Number(summary.conflicts_resolved || 0);
...
function decisionDetailEntries(details) {
  ...
  for (const [singular, plural] of [
    ...
    if (Array.isArray(list) && list.map(String).includes(String(details[singular])))
  ...
  return order
    .filter(
      key =>
        ...
        details[key] != null &&
        details[key] !== "" &&
        (!Array.isArray(details[key]) || details[key].length),
    )
    .map(key => {
...
function renderDecisionBriefs(payload) {
  ...
  document.getElementById("decision-report-link").href = safeUrl(
    `${state.data.repository_url}/blob/main/reports/bid-no-bid.md`,
  );
  document.getElementById("decision-grid").innerHTML = briefs.length
    ? briefs
        .slice(0, 6)
        .map(
          item => `
  ...
  `,
        ...
        .join("")
    : '<div class="empty-state">No provisional decision briefs are available yet.</div>';
}
...
function renderPursuits(payload) {
  ...
  document.getElementById("pursuit-report-link").href = safeUrl(
    `${state.data.repository_url}/blob/main/reports/pursuits.md`,
  );
  document.getElementById("pursuit-board").innerHTML = pursuits.length
    ? pursuits
        .slice(0, 12)
        .map(item => {
          const next =
            item.next_milestone?.name ||
            (item.managed ? "No next milestone recorded" : "Assign owner and qualify");
      ...
      ${total ? `<div class="evidence-meter"><i style="width:${Math.max(0, Math.min(100, Number(item.checklist_percent || 0)))}%"></i></div><small>${complete} of ${total} checklist items complete</small>` : ""}
        ...
        .join("")
    : '<div class="empty-state">No pursuits or qualification candidates are available yet.</div>';
}
...
function renderProcurementDocuments(payload) {
  ...
  document.getElementById("document-report-link").href = safeUrl(
    `${state.data.repository_url}/blob/main/reports/procurement-intelligence.md`,
  );
  document.getElementById("document-grid").innerHTML = opportunities.length
    ? opportunities
        .slice(0, 6)
        .map(item => {
          const activeDocuments = (item.documents || []).filter(doc => doc.active !== false);
          const extracted = activeDocuments.filter(
            doc => doc.extraction_status === "extracted",
          ).length;
      ...
      <ul>${
        (item.requirements || [])
          .slice(0, 3)
          .map(value => `<li>${escapeHtml(value)}</li>`)
          .join("") || "<li>No requirement excerpt extracted.</li>"
      }</ul>
    </article>`;
        })
        .join("")
    : '<div class="empty-state">No linked procurement documents have been analyzed yet.</div>';
}
...
function renderEvidenceLedger(claimPayload, changePayload, temporalPayload = {}) {
  ...
  document.getElementById("claim-ledger-report-link").href = safeUrl(
    `${state.data.repository_url}/blob/main/reports/claim-ledger.md`,
  );
  document.getElementById("changes-report-link").href = safeUrl(
    `${state.data.repository_url}/blob/main/reports/intelligence-changes.md`,
  );
  document.getElementById("temporal-report-link").href = safeUrl(
    `${state.data.repository_url}/blob/main/reports/temporal-intelligence.md`,
  );
  const rawChanges = [
    ...(changePayload.conflict_opened || changePayload.conflicts || []).map(item => ({
      ...item,
  ...
  const claims = (claimPayload.claims || [])
    .filter(
      item =>
    ...
    .slice(0, 8);
  document.getElementById("evidence-claim-grid").innerHTML = claims.length
    ? claims
        .map(item => {
        ...
        .join("")
    : '<div class="empty-state">No traceable claims are available yet.</div>';
}
...
function renderRelationshipExplorer(payload) {
  ...
  if (!state.relationshipMission || !byId.has(state.relationshipMission)) {
    state.relationshipMission =
      missions.find(node => (node.record_count || 0) > 0)?.node_id || missions[0]?.node_id || "";
  }
  ...
  if (!state.relationshipNode || !byId.has(state.relationshipNode))
    state.relationshipNode = state.relationshipMission;
  const nodeButton = (
    node,
    compact = false,
  ) => `<button class="relationship-node ${escapeHtml(node.node_type)}${node.node_id === state.relationshipNode ? " active" : ""}" data-node="${escapeHtml(node.node_id)}">
  ...
  const activityNodes = [...new Set(executionIds)]
    .map(id => byId.get(id))
    .filter(Boolean)
    .slice(0, 8);
  const contractorNodes = [...new Set(contractorIds)]
    .map(id => byId.get(id))
    .filter(Boolean)
    .slice(0, 6);
  const patentNodes = patentIds
    .map(id => byId.get(id))
    .filter(Boolean)
    .slice(0, 8);
  ...
  document.querySelectorAll(".relationship-node").forEach(button =>
...
function renderRelationshipDetail(graph, node) {
  ...
  const connections = (graph.edges || []).filter(
    edge => edge.source_node === node.node_id || edge.target_node === node.node_id,
  );
  const confidence = connections.reduce((counts, edge) => {
    const key = edge.confidence || "unlabeled";
    counts[key] = (counts[key] || 0) + 1;
    return counts;
  }, {});
  detail.innerHTML = `<div><span class="module-kicker">SELECTED ${escapeHtml(String(node.node_type).replaceAll("_", " "))}</span><h4>${escapeHtml(node.label || node.identifier)}</h4><p>${connections.length} connected evidence link${connections.length === 1 ? "" : "s"} · ${
    Object.entries(confidence)
      .map(([key, value]) => `${value} ${key}`)
      .join(" · ") || "no confidence label"
  }</p></div>
    <div class="relationship-evidence">${connections
      .slice(0, 5)
      .map(edge => {
...
function renderContractors(payload) {
  ...
  const visible = profiles
    ...
    .slice(0, 12);
  document.getElementById("contractor-summary").textContent =
    `${profiles.length} prioritized profiles`;
  document.getElementById("contractor-grid").innerHTML = visible.length
    ? visible
        .map(item => {
      ...
      <div class="contractor-tags">${(item.technology_specialties || [])
        .slice(0, 4)
        .map(value => `<span>${escapeHtml(value)}</span>`)
        .join("")}</div>
      <details class="contractor-profile-detail">
        ...
        <div class="contractor-profile-body">
          ...
          <section><h4>Recent awards</h4>${
            (item.top_awards || [])
              .slice(0, 3)
              .map(
                award =>
                  `<a href="${escapeHtml(safeUrl(award.url))}" target="_blank" rel="noopener"><b>${escapeHtml(formatMoney(award.amount))}</b>${escapeHtml(award.title || "Award")}</a>`,
              )
              .join("") || "<p>No award detail available.</p>"
          }</section>
          <section><h4>Related patents</h4>${
            patents
              .slice(0, 3)
              .map(
                patent =>
                  `<a href="${escapeHtml(safeUrl(patent.url))}" target="_blank" rel="noopener">${escapeHtml(patent.title || patent.patent_id)}</a>`,
              )
              .join("") || "<p>No assignee matches.</p>"
          }</section>
          <section><h4>Peers & potential competitors</h4>${
            peers
              .slice(0, 3)
              .map(
                peer =>
                  `<p><b>${escapeHtml(peer.name)}</b><span>${escapeHtml(peer.relationship_type)}</span></p>`,
              )
              .join("") || "<p>Not enough shared evidence.</p>"
          }</section>
        </div>
      </details>
    </article>`;
        ...
        .join("")
    : '<div class="empty-state">No contractor profiles match this search.</div>';
}
...
function renderSignals() {
  ...
  const themes = state.data.signals.themes.filter(item => {
    const statusMatch =
      ...
      (state.status === "priority" &&
        (item.status === "actionable" || item.importance === "critical")) ||
      item.status === state.status;
    const haystack = [
      item.name,
      ...(item.organizations || []),
      ...(item.evidence || []).map(x => x.title),
    ]
      .join(" ")
      .toLowerCase();
  ...
  document.getElementById("signal-grid").innerHTML = visibleThemes.length
    ? visibleThemes.map(signalCard).join("")
...
function renderWatch() {
  ...
  const matched = items.length
    ...
    : "<p>No configured watch items have matched evidence yet.</p>";
  const more =
    items.length > 6
      ? `<details class="watch-more"><summary>Show ${items.length - 6} more matched items</summary><div class="watch-more-grid">${items.slice(6).map(watchCard).join("")}</div></details>`
      : "";
  const awaiting = unseen.length
...
function renderReadiness() {
  ...
  document.getElementById("readiness-summary").textContent =
    `${assessed} of ${organizations.length} organizations assessed from public evidence`;
  document.getElementById("readiness-table").innerHTML =
    organizations
      .map(
        item =>
...
function renderStandards() {
  ...
  const nextText = next
    ? `${next.title} · ${next.days_remaining} day${next.days_remaining === 1 ? "" : "s"}`
    : "No upcoming milestone";
  document.getElementById("standards-summary").textContent =
    `${summary.completed || 0} completed · ${summary.due_soon || 0} due soon · next: ${nextText}`;
  const order = { overdue: 0, due_soon: 1, upcoming: 2, estimated: 3, completed: 4 };
  const milestones = [...(payload.milestones || [])].sort(
    (a, b) =>
      order[a.timing] - order[b.timing] ||
      String(a.target_date).localeCompare(String(b.target_date)),
  );
  document.getElementById("standards-timeline").innerHTML =
    milestones
      .map(item => {
        const countdown =
          item.timing === "completed"
            ? "Completed"
            : item.timing === "estimated"
              ? "Planning estimate"
...
function renderCoverage() {
  ...
  document.getElementById("coverage-summary").textContent =
    `${gaps} coverage gap${gaps === 1 ? "" : "s"} · ${coverage.length} configured organizations`;
  document.getElementById("coverage-table").innerHTML = coverage
    .map(item => {
      const sourceNames =
        (item.active_sources || []).map(source => source.name).join(", ") ||
        (item.disabled_sources || []).map(source => `${source.name} (disabled)`).join(", ") ||
        "None";
...
function renderComparison() {
  ...
  const entities = [...(watch.entities || []), ...(watch.unseen_entities || [])].sort(
    (a, b) => (b.evidence_count || 0) - (a.evidence_count || 0) || a.name.localeCompare(b.name),
  );
  if (!entities.length) {
    document.getElementById("compare-results").innerHTML =
      '<div class="empty-state">No organizations are configured for comparison.</div>';
    return;
  }
  if (!state.compareFirst || !entities.some(item => item.name === state.compareFirst))
    state.compareFirst = entities[0].name;
  if (
    !state.compareSecond ||
    state.compareSecond === state.compareFirst ||
    !entities.some(item => item.name === state.compareSecond)
  )
    state.compareSecond = (
      entities.find(item => item.name !== state.compareFirst) || entities[0]
    ).name;
  const options = selected =>
    entities
      .map(
        item =>
          `<option value="${escapeHtml(item.name)}" ${item.name === selected ? "selected" : ""}>${escapeHtml(item.name)}</option>`,
      )
      .join("");
  ...
  const readiness = new Map(
    (state.data.readiness?.organizations || []).map(item => [item.name, item]),
  );
  const selected = [state.compareFirst, state.compareSecond].map(name =>
    entities.find(item => item.name === name),
  );
  const maxEvidence = Math.max(...selected.map(item => item.evidence_count || 0), 1);
  document.getElementById("compare-results").innerHTML = selected
    .map(item => {
      const itemCoverage = coverage.get(item.name);
      const sourceHealth = (itemCoverage?.active_sources || [])
        .map(source => health.get(source.name))
        .filter(Boolean);
      const verification = sourceHealth.some(source => source.verification_status === "verified")
        ? "verified"
        : sourceHealth.some(source => source.verification_status === "failing")
          ? "failing"
          : "unverified";
      const freshness = sourceHealth.some(source => source.freshness === "fresh")
        ? "fresh"
        : sourceHealth.some(source => source.freshness === "stale")
          ? "stale"
          : "unknown";
      const activeAlerts = alerts.filter(
        alert => alert.entity?.toLowerCase() === item.name.toLowerCase(),
      ).length;
      ...
      return `<article class="compare-card"><div class="compare-head"><div><span>${escapeHtml(item.type || "organization")}</span><h3><a class="profile-link" href="${escapeHtml(profileUrl(item.name))}">${escapeHtml(item.name)}</a></h3></div><div class="badges"><span class="badge ${escapeHtml(item.priority)}">${escapeHtml(item.priority)}</span><span class="badge ${escapeHtml(item.momentum || "stable")}">${escapeHtml(item.momentum || "unseen")}</span></div></div><div class="compare-evidence"><span>Evidence volume</span><strong>${item.evidence_count || 0}</strong><div class="bar"><i style="width:${width}%"></i></div></div><dl class="compare-stats"><div><dt>Recent / prior</dt><dd>${item.recent_count || 0} / ${item.prior_count || 0}</dd></div><div><dt>Latest seen</dt><dd>${escapeHtml(item.latest_seen || "Not seen")}</dd></div><div><dt>PQC readiness</dt><dd><span class="readiness-stage ${escapeHtml(itemReadiness?.stage || "not_assessed")}">${escapeHtml(itemReadiness?.stage_label || "Not assessed")}</span></dd></div><div><dt>Active alerts</dt><dd>${activeAlerts}</dd></div><div><dt>Coverage</dt><dd>${escapeHtml(itemCoverage?.status || "N/A")}</dd></div><div><dt>Validation</dt><dd><span class="freshness ${escapeHtml(verification)}">${escapeHtml(verification)}</span></dd></div><div><dt>Freshness</dt><dd><span class="freshness ${escapeHtml(freshness)}">${escapeHtml(freshness)}</span></dd></div></dl><div class="profile-themes">${
        (item.themes || [])
          ...
          .join("") || "<span>No matched themes</span>"
      }</div><a class="watch-link" href="${escapeHtml(profileUrl(item.name))}">Open full profile →</a></article>`;
    })
    .join("");
}
...
function signalCard(item, index) {
  ...
  const evidence = (item.evidence || [])
    .map(
      x =>
        `<li><a href="${escapeHtml(safeUrl(x.url))}" target="_blank" rel="noopener">${escapeHtml(x.title)}</a> · ${escapeHtml(x.date)}</li>`,
    )
    .join("");
...
function renderDataTrust(payload) {
  ...
  document.getElementById("data-trust-summary").textContent =
    `${summary.accepted || 0} accepted · ${summary.quarantined || 0} quarantined · ${summary.acceptance_rate ?? 100}% admitted`;
  document.getElementById("data-trust-review-count").textContent = `${quarantined.length} shown`;
  document.getElementById("data-trust-report-link").href = safeUrl(
    `${state.data.repository_url}/blob/main/reports/data-trust.md`,
  );
  const metrics = payload.collector_metrics || [];
  document.getElementById("data-trust-metrics").innerHTML =
    metrics
      .map(
        item =>
      ...
      .join("") ||
    `<p class="empty-state">No evidence admission decisions have been recorded yet.</p>`;
  document.getElementById("data-trust-list").innerHTML =
    quarantined
      .map(item => {
        const admission = item.admission || {};
        const reasons = (admission.reason_codes || [])
        ...
        const titleMarkup = item.url
          ? `<a href="${escapeHtml(safeUrl(item.url))}" target="_blank" rel="noopener">${title}</a>`
          : `<strong>${title}</strong>`;
...
function renderReports(reports) {
  ...
  document.getElementById("report-cards").innerHTML = cards
    .map(
      ([type, item]) =>
        `<a class="report-card" href="${escapeHtml(safeUrl(item.url))}"><span>${type}</span><h3>${escapeHtml(friendlyReportName(item.name, type))}</h3></a>`,
    )
    .join("");
}
...
document.getElementById("status-filters").addEventListener("click", event => {
  ...
  document
    ...
    .forEach(x => x.classList.toggle("active", x === event.target));
  if (state.data) renderSignals();
});
document.getElementById("trend-ranges").addEventListener("click", event => {
  if (!event.target.dataset.days) return;
  state.trendDays = event.target.dataset.days === "all" ? "all" : Number(event.target.dataset.days);
  document
    .querySelectorAll("#trend-ranges button")
    .forEach(x => x.classList.toggle("active", x === event.target));
  if (state.data) renderTrend();
});
document.getElementById("watch-tabs").addEventListener("click", event => {
  if (!event.target.dataset.watch) return;
  state.watchType = event.target.dataset.watch;
  document
    .querySelectorAll("#watch-tabs button")
    .forEach(x => x.classList.toggle("active", x === event.target));
  if (state.data) renderWatch();
});
document.getElementById("compare-first").addEventListener("change", event => {
  state.compareFirst = event.target.value;
  if (state.data) renderComparison();
});
document.getElementById("compare-second").addEventListener("change", event => {
  state.compareSecond = event.target.value;
  if (state.data) renderComparison();
});
document.getElementById("opportunity-filters").addEventListener("click", event => {
  ...
  document
    .querySelectorAll("#opportunity-filters button")
...
document.getElementById("analyst-decision-filters").addEventListener("click", event => {
  ...
  document
    .querySelectorAll("#analyst-decision-filters button")
...
document.getElementById("analyst-decision-list").addEventListener("click", event => {
  ...
  if (current?.disposition === action) {
    ...
    const remoteItem = (state.data.decision_center?.items || []).find(
...
document.getElementById("export-analyst-decisions").addEventListener("click", () => {
  const records = Object.values(state.analystDecisions).sort((a, b) =>
    String(a.decision_id).localeCompare(String(b.decision_id)),
  );
  const content = JSON.stringify(
    { version: 1, exported_at: new Date().toISOString(), decisions: records },
    null,
    2,
  );
...
document.getElementById("clear-analyst-decisions").addEventListener("click", () => {
  if (
    !Object.keys(state.analystDecisions).length ||
    !window.confirm("Clear all analyst decisions stored in this browser?")
  )
    return;
  state.analystDecisions = {};
  try {
    window.localStorage.removeItem(ANALYST_DECISION_STORAGE_KEY);
  } catch {
    /* Browser-local storage may be unavailable. */
  }
  if (state.data) renderDecisionCenter(state.data.decision_center || {});
