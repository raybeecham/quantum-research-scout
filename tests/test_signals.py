...
class SignalTrackerTests(unittest.TestCase):
    def test_tracker_persists_and_deduplicates_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (month / "2026-07-19-digest.md").write_text(
                _daily("Migration One", "https://example.com/one", "2026-07-19", 60),
                encoding="utf-8",
            )
            (month / "2026-07-20-digest.md").write_text(
                _daily("Migration Two", "https://example.com/two", "2026-07-20", 80),
                encoding="utf-8",
            )
    ...
    def test_tracker_reconciles_corrected_retained_report_but_preserves_pruned_history(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            reports = Path(temp_dir)
            ...
            report.write_text(
                _daily("Incorrect Event", "https://example.com/old", "2026-07-20", 60),
                encoding="utf-8",
            )
            ...
            report.write_text(
                _daily("Corrected Signal", "https://example.com/new", "2026-07-20", 70),
                encoding="utf-8",
            )
            write_signal_tracker(reports, generated_at=datetime(2026, 7, 21, tzinfo=timezone.utc))
            corrected = json.loads(state_path.read_text(encoding="utf-8"))["themes"][
                "PQC / Crypto Agility"
            ]["evidence"]
            ...
            report.unlink()
            write_signal_tracker(reports, generated_at=datetime(2026, 7, 21, tzinfo=timezone.utc))
            preserved = json.loads(state_path.read_text(encoding="utf-8"))["themes"][
                "PQC / Crypto Agility"
            ]["evidence"]
