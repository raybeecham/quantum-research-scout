...
def write_alerts(
    ...
    *,
    ...
    procurement_intelligence = _read_json(reports_path / "procurement-intelligence.json")
    intelligence_changes = _read_json(reports_path / "intelligence-changes.json")
...
def _evaluate(
    ...
    for name, summary in signals.get("themes", {}).items():
        ...
        if signal_config.get("newly_actionable", True) and summary.get("status") == "actionable":
            alerts.append(
                _alert(
                    f"signal:actionable:{slug}",
                    "signal_actionable",
                    "high",
                    f"Actionable signal: {name}",
                    f"{status_icon('actionable')} {name} is actionable with "
                    f"{summary.get('confidence', 'unknown')} confidence.",
                    name,
                    "actionable",
                    "signals.md",
                )
        ...
        if signal_config.get("rising_momentum", True) and summary.get("momentum") == "rising":
            ...
            alerts.append(
                _alert(
                    f"signal:rising:{slug}",
                    "signal_rising",
                    severity,
                    f"Rising momentum: {name}",
                    f"{momentum_icon('rising')} Recent evidence is "
                    f"{summary.get('recent_count', 0)} versus {summary.get('prior_count', 0)} "
                    f"in the prior period.",
                    name,
                    "rising",
                    "signals.md",
                )
        ...
        if (
            signal_config.get("critical_importance", True)
            and summary.get("importance") == "critical"
        ):
            alerts.append(
                _alert(
                    f"signal:critical:{slug}",
                    "signal_critical",
                    "critical",
                    f"Critical theme: {name}",
                    f"{priority_icon('CRITICAL')} {name} has critical strategic importance and "
                    f"{summary.get('momentum', 'unknown')} momentum.",
                    name,
                    "critical",
                    "signals.md",
                )
...
def _procurement_document_alerts(payload: dict, config: dict) -> list[dict]:
    ...
    for opportunity in payload.get("opportunities", []):
        title = str(opportunity.get("title") or "Federal opportunity")
        opportunity_key = str(opportunity.get("opportunity_key") or opportunity.get("url") or title)
        ...
        if (
            ...
            alerts.append(
                _alert(
                    ...
                    summary.strip() or "A tracker-observed solicitation change requires review.",
            ...
            continue
        if not config.get("new_amendments", True) or not opportunity.get("new_amendment"):
            continue
        amendment = next(
            (
                document
...
def _claim_change_alerts(payload: dict, config: dict) -> list[dict]:
    ...
    for key, alert_type, default_severity in event_groups:
        ...
        for event in payload.get(key, [])[:20]:
            ...
            title = str(
                subject.get("label") or event.get("subject_label") or "Tracked intelligence claim"
            )
...
def _entity_event_alerts(entity_watch: dict, config: dict, today: date) -> list[dict]:
    ...
    for entity in entity_watch.get("entities", []):
        ...
        for evidence in entity.get("evidence", []):
            ...
            fingerprint = sha256(
                f"{entity['name']}|{event_name}|{evidence_url or title}".encode()
            ).hexdigest()[:12]
...
def _source_alert(source: dict, severity: str) -> dict:
    ...
    return _alert(
        f"source:{status}:{_slug(name)}",
        f"source_{status}",
        severity,
        f"Source {status}: {name}",
        f"{health_icon(status)} {source.get('success_rate', 0)}% reliability with "
        f"{source.get('warning_days', 0)} warning day(s).",
        name,
        status,
        "source-health.md",
    )
...
def _source_stale_alert(source: dict) -> dict:
    ...
    return _alert(
        f"source:stale:{_slug(name)}",
        "source_stale",
        "medium",
        f"Source stale: {name}",
        f"The latest dated item is from {last_item}; collection may be healthy but the content "
        f"stream is stale.",
        name,
        "stale",
        "source-health.md",
...
def _render_alerts(payload: dict) -> str:
    lines = [
        "# Intelligence Alerts",
        "",
        "> **Alert Center** · Signal transitions · Federal opportunities · Material entity "
        "events · Source degradation",
        "",
