...
fetch("data/dashboard.json?v=__ASSET_VERSION__")
  ...
  .then(renderProfile)
  .catch(error => showError(`Unable to load this profile: ${error.message}`));

function renderProfile(data) {
  const watch = data.entity_watch || {};
  const profiles = [...(watch[requestedKind] || []), ...(watch[`unseen_${requestedKind}`] || [])];
  const profile = profiles.find(
    item => item.name?.localeCompare(requestedName, undefined, { sensitivity: "accent" }) === 0,
  );
  if (!profile) {
    showError(
      requestedName
        ? `No watchlist profile was found for “${requestedName}”.`
        : "No profile was selected.",
    );
    return;
  }
  const coverage =
    requestedKind === "entities"
      ? (watch.coverage || []).find(item => item.name === profile.name)
      : null;
  const readiness =
    requestedKind === "entities"
      ? (data.readiness?.organizations || []).find(item => item.name === profile.name)
      : null;
  const alerts = (data.alerts?.alerts || []).filter(
    item => item.entity?.toLocaleLowerCase() === profile.name.toLocaleLowerCase(),
  );
  const evidence = [...(profile.evidence || [])].sort(
    (a, b) => String(b.date).localeCompare(String(a.date)) || (b.score || 0) - (a.score || 0),
  );

  document.title = `${profile.name} · Quantum Research Scout`;
  document.getElementById("repo-link").href = safeUrl(data.repository_url);
  document.getElementById("profile-kind").textContent =
    requestedKind === "entities" ? "Organization" : "Technology";
  document.getElementById("profile-eyebrow").textContent =
    `${profile.type || (requestedKind === "entities" ? "organization" : "technology")} · intelligence profile`.toUpperCase();
  ...
  document.getElementById("profile-badges").innerHTML = [
    ...
    .filter(Boolean)
    .map(value => `<span class="badge ${escapeHtml(value)}">${escapeHtml(value)}</span>`)
    .join("");
  ...
  document.getElementById("profile-momentum").textContent =
    `${profile.recent_count || 0} / ${profile.prior_count || 0}`;
  document.getElementById("profile-coverage").textContent =
    coverage?.status || (requestedKind === "technologies" ? "N/A" : "Gap");
  document.getElementById("profile-readiness").textContent =
    readiness?.stage_label || (requestedKind === "technologies" ? "N/A" : "Not assessed");
  document.getElementById("profile-updated").textContent =
    `Watch data updated ${formatDateTime(watch.updated_at)}`;
  document.getElementById("footer-updated").textContent =
    `Dashboard built ${formatDateTime(data.generated_at)} · ${data.build_id || "current build"}`;
...
function renderChart(evidence) {
  ...
  if (!evidence.length) {
    target.innerHTML =
      '<div class="empty-state">No matching evidence has been collected yet. The profile will populate automatically when a source produces a match.</div>';
    return;
  }
  ...
  if (!counts.size) {
    target.innerHTML =
      '<div class="empty-state">Authoritative evidence is available, but its publication date could not be verified. It remains visible in the timeline without being placed on the trend chart.</div>';
    return;
  }
...
function renderTimeline(evidence, repoUrl) {
  ...
  target.innerHTML = evidence.length
    ? evidence
        .map(item => {
          const report = item.report
            ? `<a href="${escapeHtml(safeUrl(`${repoUrl}/blob/main/reports/${item.report}`))}">Daily report</a>`
            : "";
          const provenance = item.historical
            ? ` · historical${item.date_kind && item.date_kind !== "unknown" ? ` · ${escapeHtml(item.date_kind)} date` : ""}`
            : "";
        ...
        .join("")
    : '<div class="empty-state">No evidence has matched this watchlist entry yet.</div>';
}

function renderThemes(themes) {
  document.getElementById("profile-themes").innerHTML = themes.length
    ? themes.map(theme => `<span>${escapeHtml(theme)}</span>`).join("")
    : '<span class="empty-state">Awaiting a matched theme</span>';
}
...
function renderReadiness(readiness, kind) {
  ...
  if (kind !== "entities") {
    target.innerHTML =
      '<div class="empty-state">Readiness stages apply to organizations, not technology profiles.</div>';
    return;
  }
  if (!readiness || readiness.stage === "not_assessed") {
    target.innerHTML =
      '<div class="empty-state">No public evidence supports a PQC readiness stage yet.</div>';
    return;
  }
  const evidence = (readiness.supporting_evidence || [])
    .slice(0, 3)
    .map(
      item =>
        `<li><a href="${escapeHtml(safeUrl(item.url))}" target="_blank" rel="noopener">${escapeHtml(item.title)}</a><small>${escapeHtml(item.observed_stage_label)} · ${escapeHtml(item.date || "undated")}${item.historical ? " · historical" : ""}</small></li>`,
    )
    .join("");
...
function renderSources(coverage, sourceHealth) {
  ...
  document.getElementById("profile-sources").innerHTML = sources.length
    ? sources
        .map(source => {
        ...
        .join("")
    : '<div class="empty-state">No official source is configured for this profile.</div>';
}

function renderAlerts(alerts) {
  document.getElementById("profile-alerts").innerHTML = alerts.length
    ? alerts
        .map(
          item =>
            `<article class="profile-alert ${escapeHtml(item.severity)}"><h4>${escapeHtml(item.title)}</h4><p>${escapeHtml(item.summary)}</p>${item.evidence_url ? `<a class="watch-link" href="${escapeHtml(safeUrl(item.evidence_url))}" target="_blank" rel="noopener">Evidence →</a>` : ""}</article>`,
        )
        .join("")
    : '<div class="empty-state">No active alerts are tied directly to this profile.</div>';
}

function showError(message) {
  document.getElementById("profile-name").textContent = "Profile unavailable";
  document.getElementById("profile-content").innerHTML =
    `<section class="section"><div class="empty-state">${escapeHtml(message)} <a class="watch-link" href="index.html#watch">Return to the watchlist.</a></div></section>`;
}
