...
def weekly_report_relative_path(start_date: date, end_date: date) -> Path:
    return (
        Path("weekly")
        / f"{start_date:%Y}"
        / f"{start_date.isoformat()}_to_{end_date.isoformat()}-weekly.md"
    )
...
def _has_daily_reports(reports_dir: Path, start_date: date, end_date: date) -> bool:
    return any(
        start_date <= report_date <= end_date for report_date in _daily_report_dates(reports_dir)
    )
...
def _latest_daily_report_date(reports_dir: Path, *, before_or_on: date) -> date | None:
    eligible_dates = [
        report_date
        for report_date in _daily_report_dates(reports_dir)
        if report_date <= before_or_on
    ]
...
def load_weekly_inputs(reports_dir: str | Path, start_date: date, end_date: date) -> WeeklyInputs:
    ...
    return WeeklyInputs(
        start_date=start_date, end_date=end_date, reports=reports, missing_dates=missing_dates
    )
...
def parse_daily_report(path: str | Path) -> ParsedDailyReport:
    ...
    if not match:
        raise ValueError(
            f"Daily report filename does not match YYYY-MM-DD-digest.md: {report_path.name}"
        )
...
def render_weekly_report(weekly: WeeklyInputs) -> str:
    ...
    lines: list[str] = [
        ...
        "|---:|---:|---:|---:|",
        f"| {len(weekly.reports)} | {len(unique_items)} | {len(weekly.missing_dates)} | "
        f"{warning_count} |",
        "",
    ]
...
def dedupe_weekly_items(items: list[WeeklyItem]) -> list[WeeklyItem]:
    ...
    for item in sorted(
        items,
        key=lambda candidate: (candidate.score, candidate.report_date or date.min),
        reverse=True,
    ):
...
def _clean_key_point(value: str, title: str) -> str:
    ...
    if normalized.startswith(
        ("tracked as", "source", "category", "published", "read more", "learn more")
    ):
        return ""
    return text
...
def _is_weak_or_incomplete_key_point(value: str) -> bool:
    ...
    if re.match(r"(?i)^\d+(?:\.\d+)?\s*dB\b", text):
        return True
    if len(text) < 35 and not _contains_any(
        lowered, FEDERAL_STRONG_TERMS | AI_SECURITY_TERMS | QEC_TERMS
    ):
        return True
    if re.search(
...
def _executive_summary(items: list[WeeklyItem], weekly: WeeklyInputs) -> list[str]:
    ...
    if items:
        top = items[0]
        bullets.append(
            f"- Top weekly signal: {_display_title(top)} from {top.source} ({top.category}, score {top.score})."
        )
    bullets.append(
        f"- Processed {len(weekly.reports)} daily report(s) covering {len(items)} unique weekly item(s)."
    )
    for theme, theme_items in _items_by_theme(items).items():
        if theme_items and len(bullets) < 7:
            bullets.append(
                f"- {theme}: {len(theme_items)} notable signal(s), led by {_display_title(theme_items[0])}."
            )
    if weekly.missing_dates:
        bullets.append(
            f"- Missing daily reports: {', '.join(day.isoformat() for day in weekly.missing_dates)}."
        )
    if len(bullets) < 5:
        bullets.append(
...
def _theme_observations(theme: str, items: list[WeeklyItem]) -> list[str]:
    ...
    if theme == "Patent Intelligence":
        ...
        if _contains_any(
            text, {"tls", "pki", "cbom", "fips", "nist", "hndl", "cryptographic inventory"}
        ):
            observations.append(
        ...
        observations.append(
            "Standards and government signals matter most where they affect compliance, procurement, and migration timelines."
        )
        observations.append(
            "Track whether these signals create new inventory, reporting, or implementation obligations."
        )
    else:
        observations.append(
            "Vendor and ecosystem movement showed where companies are investing, partnering, or positioning platforms."
        )
        observations.append(
            "Separate technical capability from funding, marketing, and partnership momentum."
        )
...
def _render_watch(
    items: list[WeeklyItem], empty_text: str, *, featured_keys: set[str]
) -> list[str]:
    if not items:
    ...
    for item in items[:12]:
        detail = (
            "featured in Top Strategic Signals"
            if _item_key(item) in featured_keys
            else _one_sentence_summary(item)
        )
        lines.append(
            f"- **{_display_title(item)}** — {_as_sentence(detail)} [Open item]({item.link})"
        )
    return lines
...
def _render_federal_implications(items: list[WeeklyItem]) -> list[str]:
    ...
    return [
        f"- **{_display_title(item)}** — {_federal_implication(item)} [Open item]({item.link})"
        for item in federal_items[:10]
    ]
...
def _render_weekly_changes(items: list[WeeklyItem], weekly: WeeklyInputs) -> list[str]:
    ...
    lines = [
        f"- The week leaned toward {', '.join(top_categories)} rather than a single isolated topic."
    ]
    if top_sources:
        lines.append(
            f"- Coverage was driven mostly by {', '.join(top_sources)}, so source mix should be considered when reading trends."
        )
    ...
    if vendor_items:
        lines.append(
            "- Vendor movement was present but should be separated from technical progress unless claims include measurable implementation detail."
        )
    unusual = [item for item in items if item.score >= 70]
    if unusual:
        lines.append(
            f"- Unusual high-impact signals: {len(unusual)} item(s) scored CRITICAL-level priority."
        )
    if weekly.missing_dates:
...
def _theme_for_item(item: WeeklyItem) -> str:
    ...
    if "network" in category or _contains_any(
        text, {"quantum networking", "qkd", "repeater", "distributed quantum"}
    ):
        return "Quantum Networking"
    if "sensing" in category or _contains_any(
        text, {"quantum sensing", "sensor", "sensing", "rf", "detection"}
    ):
        return "Quantum Sensing"
    if "tooling" in category or _contains_any(
        text, {"compiler", "toolkit", "framework", "simulator", "sdk"}
    ):
        return "Quantum Software / Tooling"
    if "hardware" in category or _contains_any(
        text, {"quantum hardware", "qubit", "processor", "superconducting", "neutral atom"}
    ):
...
def _term_in_text(text: str, term: str) -> bool:
    ...
    if re.fullmatch(r"[a-z0-9.+-]{1,4}", normalized_term):
        return (
            re.search(rf"(?<![a-z0-9]){re.escape(normalized_term)}(?![a-z0-9])", text) is not None
        )
...
def _is_vendor_movement(item: WeeklyItem) -> bool:
    title_source_category = (
        f"{clean_report_title(item.title)} {item.source} {item.category}".casefold()
    )
    evidence_text = f"{title_source_category} {' '.join(item.key_points)}".casefold()
    if "arxiv" in item.source.casefold() and not _contains_any(
        title_source_category, VENDOR_MOVEMENT_HINTS
    ):
        return False
    ...
    has_ecosystem_signal = "arxiv" not in item.source.casefold() and _contains_any(
        title_source_category, {"ecosystem", "industry"}
    )
...
def _company_topic_cluster(item: WeeklyItem) -> str:
    text = _item_text(item)
    company = next(
        (company for company in sorted(COMPANY_HINTS, key=len, reverse=True) if company in text), ""
    )
